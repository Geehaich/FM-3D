...
class PrintHead :
    ...
    def __init__(self,e_config,laser_pin_id):
        self.laser_pwm = PWMLED(laser_pin_id,
                             frequency=100)
        ...
        self.laser_perc = 0.01


    def start_laser(self, new_percent = 0):
        if new_percent:
            self.laser_perc = new_percent/100
        self.laser_pwm.value = self.laser_perc 
    def stop_laser(self):
        self.laser_pwm.off()

    def is_active(self):
        return self.laser_pwm.is_active() or self.e_motor.is_moving()
